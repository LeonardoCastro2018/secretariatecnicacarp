...
function BuscadorSimilitud({ posicion, temporada, inputCls, onVerDetalle }: {
  ...
  const LIGAS_DISPONIBLES = [
    ...
    'Paraguay 1A', 'Venezuela 1A', 'Ecuador 1A', 'México 1A',
    'USA MLS', 'España A', 'Italia A', 'Alemania A',
...
const descargarPDF = async () => {
    ...
    doc.text('SECRETARÍA TÉCNICA', 15, 13)
    doc.setFontSize(9)
    doc.setFont('helvetica', 'normal')
    doc.text('Área de Investigación y Conocimiento', 15, 20)
    ...
    doc.setFontSize(10)
    doc.setFont('helvetica', 'bold')
    doc.text('Criterio de búsqueda:', 15, 48)
    doc.setFont('helvetica', 'normal')
    doc.setFontSize(9)
    ...
    const filtrosTxt = [
      `Posición: ${posicion}`,
      `Temporada: ${temporada}`,
      minutosMin > 0 ? `Mín. ${minutosMin} min` : 'Sin mínimo',
    ...
    doc.setFontSize(7)
    doc.setFont('helvetica', 'normal')
    doc.text(`Generado: ${new Date().toLocaleDateString('es-AR')} ${new Date().toLocaleTimeString('es-AR')}  ·  ${resultados.length} jugadores  ·  Secretaría Técnica River Plate`, 15, yPos)
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
        ...
        {modo === 'descripcion' ? (
          <>
            <p className="text-sm font-medium text-gray-700">
              Describí el perfil · posición: <span className="text-red-600 font-semibold">{posicion}</span>
              <span className="ml-2 text-xs text-gray-400">(Solo jugadores sudamericanos, sin brasileros)</span>
            </p>
            <textarea
              value={descripcion}
              onChange={e => setDescripcion(e.target.value)}
              placeholder={`Ej: Quiero un ${posicion} con alto porcentaje de recuperaciones y buena llegada al gol...`}
              rows={3}
          ...
          <>
            <p className="text-sm font-medium text-gray-700">
              Jugador de referencia · posición: <span className="text-red-600 font-semibold">{posicion}</span>
        ...
        <div className="flex flex-wrap gap-4 pt-1">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            {edadMax && (
              <button onClick={() => setEdadMax('')} className="text-xs text-gray-400 hover:text-gray-600">✕</button>
            )}
          </div>
        </div>
              ...
              <div key={`${j.nombre_completo}-${idx}`} onClick={() => onVerDetalle(j)}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-gray-400">
                    {j.equipo}{j.liga ? ` · ${j.liga}` : ''}{j.edad ? ` · ${j.edad} años` : ''}{j.minutos ? ` · ${Math.round(j.minutos)} min` : ''}
                  </div>
                </div>
