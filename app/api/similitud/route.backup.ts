    ...
    try {
      const aiResp = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: 'user',
            content: `En 2 oraciones simples, explica por que estos jugadores son similares a ${jugadorReferencia} como ${posicion}. Describe que caracteristicas futbolisticas comparten. Sin markdown ni hashtags.`
          }],
        }),
      })
      const aiData = await aiResp.json()
